...
function showNowPlaying(spotifyStatusElement: HTMLElement, track: Track) {
  (spotifyStatusElement.querySelector("#nowPlaying") as HTMLElement).style
    .display = "flex";
  (spotifyStatusElement.querySelector("#lastPlayed") as HTMLElement).style
    .display = "none";
  ...
  ) as HTMLAnchorElement).href = track.artistUrl;
...
function showLastPlayed(spotifyStatusElement: HTMLElement, track: Track) {
  ...
  const lastPlayedElement = spotifyStatusElement.querySelector(
    "#lastPlayed",
  );
